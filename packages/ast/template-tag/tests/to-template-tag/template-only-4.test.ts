...
test('to-template-tag > template-only (4)', function () {
  const oldFile = normalizeFile([
    ...
    `  template_fd9b2463e5f141cfb5666b64daa1f11a(\`<ContainerQuery`,
  ...
  assert.strictEqual(
    ...
    normalizeFile([
      ...
      `}`,
      ``,
      `const ProductsProductCardComponent: TOC<ProductsProductCardSignature> = <template><ContainerQuery`,
      `  @features={{hash wide=(width min=320)}}`,
      `  @tagName="article"`,
      `  class={{styles.container}}`,
      `  data-test-product-card`,
